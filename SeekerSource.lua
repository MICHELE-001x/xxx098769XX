...
makefolder("SeekerMysticFarmer")
if not isfile("SeekerMysticFarmer/Fruits.txt") then writefile("SeekerMysticFarmer/Fruits.txt", "false") end

local settings = getgenv().SeekerMysticFarmerConfig
...
fileName = string.format("SeekerMysticFarmer/%s_servers.json", tostring(PlaceId))
ServerHopData = { 
    CheckedServers = {},
    LastTimeHop = nil,
    CreatedAt = os.time()
...
if (math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) <= settings.Fruits.StartFarmAt) or (readfile("SeekerMysticFarmer/Fruits.txt") == "true") then
    writefile("SeekerMysticFarmer/Fruits.txt", "true")

    if math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) >= settings.Fruits.Max then
        writefile("SeekerMysticFarmer/Fruits.txt", "false")
    ...
    while true do
        ...
        if settings.Areas["Mystic Mine"].GiantChest then
            ...
            elseif settings.MineMode == "All" then
                ...
                game.Players.LocalPlayer:Kick("Invalid mining mode (SeekerMysticFarmer)")
            end
        ...
        task.wait(1)
        if (getFruit"Orange" <= settings.Fruits.StartFarmAt) or (readfile("SeekerMysticFarmer/Fruits.txt") == "true") then break end
    end
