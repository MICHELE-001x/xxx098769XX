...
makefolder("SeekerDiamondsFarmer")
if not isfile("SeekerDiamondsFarmer/Fruits.txt") then writefile("SeekerDiamondsFarmer/Fruits.txt", "false") end

local settings = getgenv().SeekerDiamondsFarmerConfig
...
fileName = string.format("SeekerDiamondsFarmer/%s_servers.json", tostring(PlaceId))
ServerHopData = { 
    CheckedServers = {},
    LastTimeHop = nil,
    CreatedAt = os.time()
...
if (math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) <= settings.Fruits.StartFarmAt) or (readfile("SeekerDiamondsFarmer/Fruits.txt") == "true") then
    writefile("SeekerDiamondsFarmer/Fruits.txt", "true")

    if math.ceil(Library.FruitCmds.Get(game.Players.LocalPlayer, Library.FruitCmds.Directory.Orange)) >= settings.Fruits.Max then
        writefile("SeekerDiamondsFarmer/Fruits.txt", "false")
    ...
    while true do
        ...
        if settings.Areas["Mystic Mine"].GiantChest then
            ...
            elseif settings.MineMode == "All" then
                ...
                game.Players.LocalPlayer:Kick("Invalid mining mode (SeekerDiamondsFarmer)")
            end
        ...
        task.wait(1)
        if (getFruit"Orange" <= settings.Fruits.StartFarmAt) or (readfile("SeekerDiamondsFarmer/Fruits.txt") == "true") then break end
    end
